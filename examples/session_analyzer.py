...
Importable:
    from examples.session_analyzer import run_analysis
...
def run_analysis(log_path: str | Path = "examples/stderr.log", max_steps: int = 20) -> Path | None:
    ...
    if not log_path.exists():
        print(f"[session_analyzer] log file not found: {log_path}", file=sys.stderr)
        return None

    print(f"[session_analyzer] parsing log: {log_path}", file=sys.stderr)
    session = parse_session_log(log_path)

    if not session.session_id:
        session.session_id = "unknown"
        print("[session_analyzer] warning: could not extract session ID — using 'unknown'", file=sys.stderr)

    print(
        f"[session_analyzer] session={session.session_id}  "
...
"""
    ...
    if output_path.exists():
        print(f"[session_analyzer] report → {output_path} ({output_path.stat().st_size:,} bytes)", file=sys.stderr)
        return output_path

    print(f"[session_analyzer] warning: agent did not write {output_path}", file=sys.stderr)
    return None
