...
class _SummaryStyle:
    ...
    def title_row(self, text: str) -> str:
        ...
        return self.BORDER + "║" + " " * pad_l + self.VALUE + text + self.R + " " * pad_r + self.BORDER + "║" + self.R

    def metric_row1(self, lbl: str, val: str) -> str:
        """Full-width single metric row."""
        vis = f"  {lbl}: {val}"
        pad = self.W - len(vis)
        inner = f"  {self.LABEL}{lbl}{self.R}: {self.VALUE}{val}{self.R}" + " " * max(pad, 0)
...
def _handoff_rows(s: _SummaryStyle, index: int, h: dict, context_window: int | None) -> list[str]:
...
def _tool_calls_section(s: _SummaryStyle, records: list) -> list[str]:
    ...
    for i, r in enumerate(records, start=1):
        lines.extend(_tool_call_rows(
            s, i, r["name"], r["args"], r["ok"],
            status=r.get("status", ""),
...
def _context_resets_section(s: _SummaryStyle, handoffs: list, context_window: int | None) -> list[str]:
    """Box rows for the CONTEXT RESETS section (one block per handoff)."""
    if not handoffs:
        return [
            s.empty_row(), s.mid, s.empty_row(),
            s.metric_row1("CONTEXT RESETS", "0  (no reset triggered)"),
        ]
    lines = [s.empty_row(), s.mid, s.empty_row(),
             s.metric_row1("CONTEXT RESETS", str(len(handoffs))), s.empty_row()]
