...
@dataclass
class TreeNode:
    """One node in the reconstructed pipeline trace tree.

    ``type`` is a stable kind (``agent``/``step``/``context_manager``/
    ``tool_router``/``llm_call``/``ai_reasoning``/``ai_content``/``user_message``/
    ``system_message``/``tool_executor``/``tool_call``/``tool_output_validation``)
    named after the real classes/functions it represents — the whole pipeline is
    harness-managed, so there is no per-node "creator". ``node_id`` is a stable
    dotted path used by the viewer for selection and collapse state.
    """
    ...
    def add(self, child: TreeNode) -> TreeNode:
        """Append a child (assigning its dotted ``node_id``) and return it."""
        child.node_id = f"{self.node_id}.{len(self.children)}"
        self.children.append(child)
        return child

    def to_dict(self) -> dict[str, Any]:
        """Serialize the node (and its subtree) to a JSON-friendly dict."""
        return {
