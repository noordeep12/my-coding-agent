"""Terminal-UI renderers: startup banner and end-of-run summary box.

Owns the rich box-drawn ``print_banner`` and ``print_run_summary`` renderers and
their row/section/chart/markdown sub-helpers, plus the ``_git_branch`` subprocess
helper shared by both. Pure presentation — it reads no logging state and only writes
formatted text to ``sys.stderr``.
"""
