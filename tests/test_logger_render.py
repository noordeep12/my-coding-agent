...
from my_coding_agent.utils.terminal_ui import (
...
def test_print_banner_contains_labels_and_values(monkeypatch):
    ...
    monkeypatch.setattr("my_coding_agent.utils.terminal_ui._git_branch", lambda: "main")
    tools = [
...
def test_tool_call_rows_single_line_ok_marker():
    s = _SummaryStyle()
    rows = _tool_call_rows(s, 1, "bash", {"command": "ls"}, ok=True)
    assert len(rows) == 1
    plain = _plain(rows[0])
...
def test_tool_call_rows_failed_marker():
    s = _SummaryStyle()
    rows = _tool_call_rows(s, 2, "bash", {}, ok=False)
    assert "✗" in _plain(rows[0])


def test_tool_call_rows_skipped_marker():
    s = _SummaryStyle()
    rows = _tool_call_rows(s, 3, "bash", {}, ok=False, status="skipped")
    assert "⊘" in _plain(rows[0])


def test_tool_call_rows_badges_appear():
    s = _SummaryStyle()
    rows = _tool_call_rows(s, 1, "bash", {}, ok=True, artifact=True, truncated=True)
...
def test_tool_call_rows_long_args_wrap_to_multiple_rows():
    ...
    rows = _tool_call_rows(s, 1, "bash", args, ok=True)
    assert len(rows) > 1
...
def test_tool_calls_section_one_row_per_record():
    ...
    rows = _tool_calls_section(s, records)
    joined = _plain("".join(rows))
...
def test_token_chart_rows_no_main_calls():
    ...
    rows = _token_chart_rows(s, internal)
...
def test_token_chart_rows_renders_chart_for_main_calls():
    ...
    rows = _token_chart_rows(s, usage)
    joined = _plain("".join(rows))
...
def test_handoff_rows_three_lines_with_values():
    s = _SummaryStyle()
    rows = _handoff_rows(s, 1, _handoff(), context_window=10000)
    assert len(rows) == 3
    joined = _plain("".join(rows))
...
def test_handoff_rows_truncates_long_path():
    ...
    rows = _handoff_rows(s, 1, h, context_window=10000)
    assert "…" in _plain("".join(rows))


def test_context_resets_section_no_handoffs():
    s = _SummaryStyle()
    rows = _context_resets_section(s, [], context_window=10000)
    assert "no reset triggered" in _plain("".join(rows))


def test_context_resets_section_with_handoffs():
    s = _SummaryStyle()
    rows = _context_resets_section(s, [_handoff(), _handoff(3)], context_window=10000)
    joined = _plain("".join(rows))
